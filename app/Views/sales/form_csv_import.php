...
<?= form_open_multipart('sales/importCsvFile/', ['id' => 'csv_form', 'class' => 'form-horizontal']) ?>
    <fieldset id="sales_basic_info">

        <div class="form-group form-group-sm">
            <div class="col-xs-12">
                <a href="<?= esc('sales/generateCsvFile', 'attr') ?>"><?= lang('Common.download_import_template') ?></a>
            </div>
        </div>
            ...
            submitHandler: function(form) {
                $(form).ajaxSubmit({
                    success: function(response) {
                        dialog_support.hide();
                        table_support.handle_submit('<?= esc('sales') ?>', response);
                    },
                    dataType: 'json'
                });
            },
