...
<div id="register_wrapper">
    ...
    <?= form_open("$controller_name/add", ['id' => 'add_item_form', 'class' => 'form-horizontal panel panel-default']) ?>
        <div class="panel-body form-group">
            <ul>
                <li class="pull-left">
                    <?= form_input(['name' => 'item', 'id' => 'item', 'class' => 'form-control input-sm dotco-hidden-scanner-input', 'size' => '50', 'tabindex' => ++$tabindex]) ?>
			...
			<div id="dotco_item_cards" class="dotco-item-cards print_hide" aria-label="Quick item picker">
				<div class="dotco-item-cards__header">
					<strong>Quick items</strong>
					<div class="dotco-item-cards__actions">
						<div class="input-group input-group-sm dotco-item-cards__search" role="search">
							<span class="input-group-addon"><span class="glyphicon glyphicon-search"></span></span>
							<input type="text" class="form-control" id="dotco_item_cards_search" placeholder="Search items..." autocomplete="off">
							<span class="input-group-btn">
								<button type="button" class="btn btn-default" id="dotco_item_cards_clear" title="Clear">×</button>
							</span>
						</div>
						<div class="btn-group btn-group-sm" role="group" aria-label="Pagination">
							<button type="button" class="btn btn-default btn-xs" id="dotco_item_cards_first">«</button>
							<button type="button" class="btn btn-default btn-xs" id="dotco_item_cards_prev">‹</button>
							<button type="button" class="btn btn-default btn-xs" id="dotco_item_cards_next">›</button>
							<button type="button" class="btn btn-default btn-xs" id="dotco_item_cards_last">»</button>
						</div>
						<span id="dotco_item_cards_meta" class="dotco-item-cards__meta"></span>
					</div>
				</div>
				<ul id="dotco_item_cards_categories" class="nav nav-pills dotco-item-cards__categories" role="tablist"></ul>
				<div id="dotco_item_cards_grid" class="dotco-item-cards__grid"></div>
			</div>
        </div>
    <?= form_close() ?>
...
<script type="text/javascript">
    (function() {
        const registerUrl = "<?= site_url('sales'); ?>";

        const refreshRegister = function() {
            console.log('[dotco] Refreshing register (preserving item cards grid)...');
            return $.get(registerUrl).done(function(resp) {
                const $newCart = $(resp).find('#register');
                const $newOverallSale = $(resp).find('#overall_sale');

                if ($newCart.length) {
                    $('#register').replaceWith($newCart);
                    console.log('[dotco] Cart table replaced');
                }
                if ($newOverallSale.length) {
                    $('#overall_sale').replaceWith($newOverallSale);
                    console.log('[dotco] Overall sale section replaced');
                    
                    // Reinitialize Bootstrap selectpicker for payment type dropdown
                    if ($('#payment_types').length) {
                        $('#payment_types').selectpicker('destroy').selectpicker();
                        console.log('[dotco] Payment type selectpicker reinitialized');
                    }
                    
                    // Reinitialize payment type handlers
                    if (typeof check_payment_type === 'function') {
                        check_payment_type();
                        console.log('[dotco] Payment type handlers reinitialized');
                    }
                    
                    // Reinitialize customer autocomplete
                    if ($('#customer').length) {
                        $('#customer').autocomplete('destroy').autocomplete({
                        ...
                        console.log('[dotco] Customer autocomplete reinitialized');
                    }
                    
                    // Rebind customer keypress handler
                    $('#customer').off('keypress').on('keypress', function(e) {
                        if (e.which == 13) {
                            $('#select_customer_form').submit();
                            return false;
                        }
                    ...
                    // Rebind customer blur handler
                    $('#customer').off('blur').on('blur', function() {
                        $(this).val("<?= lang(ucfirst($controller_name) . '.start_typing_customer_name') ?>");
                    });
                    
                    // Rebind customer click/dblclick handlers
                    ...
                    $('#customer').off('click').click(clear_fields).off('dblclick').dblclick(function(event) {
                        $(this).autocomplete('search');
                    });
                    
                    // Rebind payment type change handler
                    $('#payment_types').off('change').change(check_payment_type);
                    
                    // Rebind amount_tendered keypress handler
                    $('#amount_tendered').off('keypress').on('keypress', function(event) {
                        if (event.which == 13) {
                            $('#add_payment_form').submit();
                        }
                    });
                    
                    // Rebind finish_sale_button keypress handler
                    $('#finish_sale_button').off('keypress').on('keypress', function(event) {
                        if (event.which == 13) {
                            $('#finish_sale_form').submit();
                        }
                    });
                    
                    console.log('[dotco] All overall_sale event handlers reinitialized');
                }

                if (!$newCart.length && !$newOverallSale.length) {
                    console.warn('[dotco] Could not find cart or overall_sale in response, falling back to full reload');
                    window.location.href = registerUrl;
                } else {
                    console.log('[dotco] Register refreshed successfully');
                }
            }).fail(function() {
                console.error('[dotco] Failed to refresh register, falling back to full reload');
                window.location.href = registerUrl;
            });
        };

        const ajaxAddItem = function() {
            const $form = $('#add_item_form');
            $form[0].submit();
        };

        window.dotco_register_init = function() {
            const redirect = function() {
                window.location.href = registerUrl;
            };

            console.log('[dotco] Initializing register handlers...');

            // Prevent double-binding by resetting handlers on the wrapper.
            $('#register_wrapper').off();
            $(document).off('submit.dotco_add_item');

            // AJAX add item (scan/enter/card click/autocomplete)
            $(document).on('submit.dotco_add_item', '#add_item_form', function(e) {
                e.preventDefault();
                ajaxAddItem();
                return false;
            });

            $('#register_wrapper').on('click', '#remove_customer_button', function() {
                $.post("<?= site_url('sales/removeCustomer'); ?>", refreshRegister);
            });

            $('#register_wrapper').on('click', '.delete_item_button', function() {
                const item_id = $(this).data('item-id');
                $.post("<?= site_url('sales/deleteItem/'); ?>" + item_id, refreshRegister);
            });

            $('#register_wrapper').on('click', '.delete_payment_button', function() {
                const item_id = $(this).data('payment-id');
                $.post("<?= site_url('sales/deletePayment/'); ?>" + item_id, refreshRegister);
            });

            $('#register_wrapper').on('change', "input[name='item_number']", function() {
            ...
            $('#register_wrapper').on('change', "input[name='name']", function() {
                var item_id = $(this).parents('tr').find("input[name='item_id']").val();
            ...
            $('#register_wrapper').on('change', "input[name='item_description']", function() {
                var item_id = $(this).parents('tr').find("input[name='item_id']").val();
		...
		let dotcoCardsPage = 1;
		const dotcoCardsLimit = 24;
		let dotcoCardsTotalPages = 1;
		let dotcoCardsCategory = '';
		let dotcoCardsInFlight = null;
		let dotcoCardsDebounce = null;

		const getCardsQuery = function() {
			return ($('#dotco_item_cards_search').val() || '').trim();
		};

		const setCardsLoading = function(isLoading) {
			$('#dotco_item_cards_grid').toggleClass('is-loading', !!isLoading);
		};

		const renderCards = function(payload) {
			const $grid = $('#dotco_item_cards_grid');
			$grid.empty();

			if (!payload || !payload.rows || payload.rows.length === 0) {
				$grid.append('<div class="dotco-item-cards__empty">No items found</div>');
				return;
			}

			payload.rows.forEach(function(row) {
				const safeName = $('<div>').text(row.name || '').html();
				const safePrice = $('<div>').text(row.price_formatted || '').html();
				const img = row.thumb_url
					? '<img class="dotco-item-card__img" src="' + row.thumb_url + '" alt="" loading="lazy">'
					: '<div class="dotco-item-card__img dotco-item-card__img--placeholder" aria-hidden="true"></div>';

				$grid.append(
					'<button type="button" class="btn btn-default dotco-item-card" data-item-value="' + (row.value || '') + '">' +
						'<div class="dotco-item-card__media">' + img + '</div>' +
						'<div class="dotco-item-card__meta">' +
							'<div class="dotco-item-card__name">' + safeName + '</div>' +
							'<div class="dotco-item-card__price">' + safePrice + '</div>' +
						'</div>' +
					'</button>'
				);
			});
		...
		const loadCards = function() {
			if (dotcoCardsInFlight && dotcoCardsInFlight.readyState !== 4) {
				dotcoCardsInFlight.abort();
			}

			const q = getCardsQuery();
			$('#dotco_item_cards_meta').text('');
			setCardsLoading(true);

			dotcoCardsInFlight = $.ajax({
				url: "<?= esc("$controller_name/itemCards") ?>",
				dataType: 'json',
				data: {
					q: q,
					category: dotcoCardsCategory,
					page: dotcoCardsPage,
					limit: dotcoCardsLimit
				}
			}).done(function(resp) {
				dotcoCardsTotalPages = resp && resp.total_pages ? parseInt(resp.total_pages, 10) : 1;
				dotcoCardsPage = resp && resp.page ? parseInt(resp.page, 10) : dotcoCardsPage;
				const from = resp && typeof resp.from !== 'undefined' ? resp.from : 0;
				const to = resp && typeof resp.to !== 'undefined' ? resp.to : 0;
				const total = resp && typeof resp.total !== 'undefined' ? resp.total : 0;
				$('#dotco_item_cards_meta').text(from + '-' + to + ' of ' + total + ' (Page ' + dotcoCardsPage + '/' + dotcoCardsTotalPages + ')');
				$('#dotco_item_cards_prev').prop('disabled', !(resp && resp.has_prev));
				$('#dotco_item_cards_first').prop('disabled', dotcoCardsPage <= 1);
				$('#dotco_item_cards_next').prop('disabled', !(resp && resp.has_next));
				$('#dotco_item_cards_last').prop('disabled', dotcoCardsPage >= dotcoCardsTotalPages);
				renderCards(resp);
			}).always(function() {
		...
		const renderCategories = function(payload) {
			const $list = $('#dotco_item_cards_categories');
			$list.empty();
			const cats = (payload && payload.categories) ? payload.categories : [];

			const makeLi = function(label, value, isActive) {
				const activeClass = isActive ? 'active' : '';
				return '<li role="presentation" class="' + activeClass + '">' +
					'<a href="#" role="tab" data-category="' + (value || '') + '">' +
						label +
					'</a>' +
				'</li>';
			};

			$list.append(makeLi('All', '', dotcoCardsCategory === ''));
			cats.forEach(function(c) {
				const name = c && c.name ? c.name : '';
				const safeName = $('<span>').text(name).html();
				const safeCount = c && typeof c.count !== 'undefined' ? $('<span>').text(String(c.count)).html() : '';
				const label = safeCount !== ''
					? (safeName + ' <span class="badge">' + safeCount + '</span>')
					: safeName;
				$list.append(makeLi(label, name, dotcoCardsCategory === name));
			});
		};

		const loadCategories = function() {
			$.ajax({
				url: "<?= esc("$controller_name/itemCardCategories") ?>",
				dataType: 'json',
				data: { q: getCardsQuery() }
			}).done(function(resp) {
				renderCategories(resp);
			});
		};

		$('#dotco_item_cards_prev').on('click', function() {
			if (dotcoCardsPage > 1) {
				dotcoCardsPage -= 1;
				loadCards();
			}
		...
		$('#dotco_item_cards_first').on('click', function() {
			dotcoCardsPage = 1;
			loadCards();
		});

		$('#dotco_item_cards_next').on('click', function() {
			if (dotcoCardsPage < dotcoCardsTotalPages) {
				dotcoCardsPage += 1;
				loadCards();
			}
		});

		$('#dotco_item_cards_last').on('click', function() {
			dotcoCardsPage = dotcoCardsTotalPages;
			loadCards();
		});

		$('#dotco_item_cards_categories').on('click', 'a[data-category]', function(e) {
			e.preventDefault();
			dotcoCardsCategory = $(this).data('category') || '';
			dotcoCardsPage = 1;
			$('#dotco_item_cards_categories li').removeClass('active');
			$(this).parent('li').addClass('active');
			loadCards();
		});

		$('#dotco_item_cards_grid').off('click').on('click', '.dotco-item-card', function() {
		...
		$('#dotco_item_cards_clear').off('click').on('click', function() {
			$('#dotco_item_cards_search').val('');
			dotcoCardsCategory = '';
			dotcoCardsPage = 1;
			loadCategories();
			loadCards();
		...
		$('#dotco_item_cards_search').off('input').on('input', function() {
			if (dotcoCardsDebounce) {
				clearTimeout(dotcoCardsDebounce);
			}
			dotcoCardsDebounce = setTimeout(function() {
				dotcoCardsPage = 1;
				loadCategories();
				loadCards();
			}, 250);
        ...
        $('#item').off('keypress').on('keypress', function(e) {
            if (e.which == 13) {
                $('#add_item_form').trigger('submit');
                return false;
            }
        ...
        $('[name="price"],[name="quantity"],[name="discount"],[name="description"],[name="serialnumber"],[name="discounted_total"]').off('change').on('change', function() {
            $(this).parents('tr').prevAll('form:first').submit()
        });

        $('[name="discount_toggle"]').off('change').on('change', function() {
        ...
        $(document).ready(function() {
            console.log('[dotco] Document ready, initializing register...');
            window.dotco_register_init();
        });
    })();
