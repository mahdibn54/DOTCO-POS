...
class Sales extends Secure_Controller
{
    ...
    public function getItemCards(): void
    {
        helper(['url', 'locale']);
        ...
        $page = (int)$this->request->getGet('page', FILTER_SANITIZE_NUMBER_INT);
        ...
        $total = $this->item->get_card_items_count($search, $category);
        ...
        $offset = ($page - 1) * $limit;
        $items = $this->item->get_card_items($search, $category, $limit, $offset);

        $rows = [];
        foreach ($items as $item) {
            $value = !empty($item['item_number']) ? $item['item_number'] : (string)$item['item_id'];
            $thumb_url = '';

            if (!empty($item['pic_filename'])) {
                $filename = $item['pic_filename'];
                $ext = pathinfo($filename, PATHINFO_EXTENSION);
                $glob_pattern = empty($ext)
                    ? FCPATH . 'uploads/item_pics/' . $filename . '.*'
                    : FCPATH . 'uploads/item_pics/' . $filename;

                $images = glob($glob_pattern);
                if (sizeof($images) > 0) {
                    $basename = pathinfo($images[0], PATHINFO_BASENAME);
                    $thumb_url = site_url('items/PicThumb/' . $basename);
                }
            }
            ...
            $rows[] = [
                ...
                'price_formatted' => to_currency((string)$item['unit_price']),
                'thumb_url'       => $thumb_url
            ];
        }
        ...
        $to = $total === 0 ? 0 : min($offset + $count, $total);
    ...
    public function getItemCardCategories(): void
    {
        $search = $this->request->getGet('q', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
        $rows = $this->item->get_card_categories($search);
        $categories = [];
